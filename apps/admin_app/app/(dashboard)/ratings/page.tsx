...
export default async function RatingsPage() {
  ...
  const { data, error } = await supabase
    .from('driver_ratings')
    .select(
      'rating_id, trip_id, rider_id, driver_id, rating, comment, created_at',
    )
    ...
    <div className='space-y-4'>
      <div>
        ...
        <p className='mt-1 text-sm muted'>
          Driver ratings and reviews (admin view).
        </p>
      </div>
        ...
        <table className='w-full text-sm'>
          ...
          <tbody>
              ...
              >
                <td className='px-4 py-3'>
                  ...
                  <div className='text-xs muted'>
                    Trip ending {r.trip_id.slice(-6)}
                  </div>
                </td>
                <td className='px-4 py-3'>
                  {r.comment ?? <span className='text-xs muted'>—</span>}
                </td>
            ...
            {!rows.length ? (
              <tr>
                <td
                  className='px-4 py-10 text-center text-sm muted'
                  colSpan={3}
                >
                  No ratings yet.
                </td>
