...
export default async function VerificationPage({
  ...
  async function review(formData: FormData) {
    ...
    logActionInfo('admin.verification.review', 'started', {
      docId,
      decision,
    ...
    if (!docId || (decision !== 'APPROVED' && decision !== 'DECLINED')) {
      logActionWarn('admin.verification.review', 'invalid_request', {
        docId,
        decision,
      });
    ...
    if (!reason.trim()) {
      logActionWarn('admin.verification.review', 'missing_reason', {
        docId,
        decision,
      });
    ...
    if (updateErr) {
      logActionError('admin.verification.review', 'update_failed', updateErr, {
        docId,
        decision,
        userId: user.id,
      });
    ...
    await supabase.rpc('admin_audit_log', {
      p_action:
        decision === 'APPROVED' ? 'document.approve' : 'document.decline',
    ...
    logActionInfo('admin.verification.review', 'completed', {
      docId,
      decision,
      userId: user.id,
    });
  ...
  async function approveDriver(formData: FormData) {
    ...
    if (userErr || !user) {
      console.error('[approveDriver] Not authenticated', {
        targetDriverId,
        userErr,
      });
      return { ok: false as const, error: 'Not authenticated' };
    ...
    if (vehiclesErr) {
      console.error('[approveDriver] Vehicle fetch failed', {
        targetDriverId,
        vehiclesErr,
      });
    ...
    if (docsErr) {
      console.error('[approveDriver] Doc fetch failed', {
        targetDriverId,
        docsErr,
      });
    ...
    if (!docs.length) {
      console.error('[approveDriver] No docs found', {
        targetDriverId,
    ...
    if (hasPending) {
      ...
      console.error('[approveDriver] Pending docs exist', {
        targetDriverId,
        pendingCount,
    ...
    if (updateErr) {
      console.error('[approveDriver] Profile update failed', {
        targetDriverId,
        updateErr,
      });
      return { ok: false as const, error: userFacingError(updateErr) };
  ...
  const queueDriverIds = new Set(
    queueRows.filter((d) => d.entity_type === 'DRIVER').map((d) => d.entity_id),
  );
  ...
  const { data: vehiclesForDriver } = driverId
    ? await supabase
        .from('vehicles')
        .select('vehicle_id, linked_driver_id')
        .eq('linked_driver_id', driverId)
    : { data: [] as unknown[] };

  const vehicleIdsForDrivers = new Set<string>();
  for (const v of (vehiclesForDriver ?? []) as unknown as Array<{
  ...
  const entityIdsToFetch = Array.from(
    new Set([
      ...driverIdsArray,
      ...vehicleIdsFromQueueDocs,
      ...Array.from(vehicleIdsForDrivers),
    ]),
  );
  ...
  const driverIdsFromDriverDocs = new Set(
    docsWithUrls
  ...
  const vehicles = ((vehiclesRaw ?? []) as unknown as Array<VehicleRow>).filter(
    (v) => !!v.vehicle_id,
  );
  const profiles = ((profilesRaw ?? []) as unknown as Array<ProfileRow>).filter(
    (p) => !!p.id,
  );

  const driverIdsFromVehicleDocs = new Set(
    vehicles.map((v) => v.linked_driver_id).filter(Boolean) as Array<string>,
  );
  const missingProfileIds = Array.from(driverIdsFromVehicleDocs).filter(
  ...
  for (const d of docsWithUrls) {
    ...
    if (d.entity_type === 'VEHICLE') {
      ...
      if (vehicle?.linked_driver_id)
        driverIdsForCases.add(vehicle.linked_driver_id);
    }
  }

  const cases: Array<DriverVerificationCase> = Array.from(
    driverIdsForCases,
  ).map((driverId) => {
    const driverDocs = docsWithUrls.filter(
      (d) => d.entity_type === 'DRIVER' && d.entity_id === driverId,
    );
    ...
    const allDocs = [
      ...driverDocs,
      ...Object.values(vehicleDocsByVehicleId).flat(),
    ];
    ...
    const oldestCreatedAt =
      allDocs.reduce<string | null>((acc, d) => {
        ...
        return new Date(d.created_at).getTime() < new Date(acc).getTime()
          ? d.created_at
          : acc;
      }, null) ?? null;
  ...
  cases.sort((a, b) => {
    const ta = a.stats.oldestCreatedAt
      ? new Date(a.stats.oldestCreatedAt).getTime()
      : Number.POSITIVE_INFINITY;
    const tb = b.stats.oldestCreatedAt
      ? new Date(b.stats.oldestCreatedAt).getTime()
      : Number.POSITIVE_INFINITY;
    return ta - tb;
  });
