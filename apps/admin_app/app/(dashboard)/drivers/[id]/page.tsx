...
export default async function DriverDetailPage({
  ...
  const entityIds = [p.id, vehicle?.vehicle_id].filter(Boolean) as Array<string>;
  ...
  const baseDocs = ((docsRaw ?? []) as unknown as Array<Omit<DocRow, "signedUrl">>).map((d) => ({
    ...d,
    signedUrl: null,
  })) as Array<DocRow>;
  ...
  const selectedDoc = docs.find((d) => d.document_id === docId) ?? docs[0] ?? null;
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-2 md:flex-row md:items-end md:justify-between">
        <div>
          ...
          <h1 className="mt-1 text-2xl font-semibold tracking-tight">{p.full_name ?? "Unnamed driver"}</h1>
          <p className="mt-1 text-sm muted">Operational overview, documents, trips, ratings, and audit history.</p>
        </div>
        <div className="flex flex-wrap gap-2">
          <Link className="rounded-lg border border-token surface-1 px-3 py-2 text-sm font-semibold" href="/drivers">
            Back
          </Link>
          <Link className="rounded-lg bg-[var(--brand-red)] px-3 py-2 text-sm font-semibold text-white hover:brightness-95" href="/verification">
            Verification
          </Link>
        </div>
      </div>
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        <Link
          href={tabHref(p.id, "documents", selectedDoc?.document_id ?? undefined)}
          className={[
            "rounded-full border px-3 py-1.5 text-sm font-semibold transition",
            activeTab === "documents"
              ? "border-[var(--brand-red)] bg-[var(--brand-red)] text-white"
      ...
      {activeTab === "documents" ? (
        <div className="grid grid-cols-1 gap-3 lg:grid-cols-[340px_1fr]">
          <div className="rounded-2xl border border-token surface-1 p-4 shadow-[var(--shadow)]">
            ...
            <div className="mt-1 text-xs muted">Driver + linked vehicle documents</div>
                  ...
                  >
                    <div className="flex items-start justify-between gap-2">
                      <div className="min-w-0">
                        ...
                        <div className="mt-1 text-xs muted">{d.entity_type} • {fmt(d.created_at)}</div>
                      </div>
                      <div className="shrink-0 space-y-1 text-right">
                        <StatusChip label={(d.status ?? "—").toLowerCase()} tone={docTone(d)} />
                        {exp ? <StatusChip label={exp.label} tone={exp.tone} /> : null}
                      </div>
                    </div>
                  </Link>
                );
              })}
              {!docs.length ? <div className="rounded-xl border border-token bg-black/2 px-3 py-3 text-sm muted">No documents found.</div> : null}
            </div>
          </div>
          ...
          <div className="rounded-2xl border border-token surface-1 p-4 shadow-[var(--shadow)]">
            <div className="flex items-start justify-between gap-3">
              <div className="min-w-0">
                <div className="truncate text-sm font-semibold">{selectedDoc ? selectedDoc.document_type : "Select a document"}</div>
        ...
        <div className="rounded-2xl border border-token surface-1 p-6 shadow-[var(--shadow)]">
          ...
          <p className="mt-2 text-sm muted">
            Trip drill-down and intervention actions will appear here once the trips filtering + intervention RPCs are finalized.
          </p>
          <Link className="mt-4 inline-flex rounded-lg border border-token px-3 py-2 text-sm font-semibold" href="/trips">
            Open trips
          </Link>
        </div>
      ) : activeTab === "ratings" ? (
        <div className="rounded-2xl border border-token surface-1 p-6 shadow-[var(--shadow)]">
          ...
          <p className="mt-2 text-sm muted">Ratings distribution and recent reviews will appear once the ratings schema lands.</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 gap-3 lg:grid-cols-3">
          <div className="rounded-2xl border border-token surface-1 p-4 shadow-[var(--shadow)]">
            ...
            <div className="mt-1 text-xs muted">Online: {p.online_status ?? "—"}</div>
            <div className="mt-3 flex flex-wrap gap-2">
              <StatusChip label={p.training_completed ? "training done" : "training no"} tone={p.training_completed ? "ok" : "warn"} />
              <StatusChip label={p.registration_submitted ? "submitted" : "not submitted"} tone={p.registration_submitted ? "ok" : "muted"} />
            </div>
          </div>
          ...
          <div className="rounded-2xl border border-token surface-1 p-4 shadow-[var(--shadow)]">
            ...
            <div className="mt-3 text-xs muted">Created {fmt(p.created_at)} • Updated {fmt(p.updated_at)}</div>
          </div>
          ...
          <div className="rounded-2xl border border-token surface-1 p-4 shadow-[var(--shadow)]">
            ...
            {vehicle ? (
              <div className="mt-2">
                <div className="text-sm font-semibold">{vehicle.registration_number ?? "—"}</div>
                <div className="mt-1 text-xs muted">{`${vehicle.make ?? ""} ${vehicle.model ?? ""}`.trim() || "—"}</div>
                <div className="mt-3 flex flex-wrap gap-2">
                  <StatusChip label={(vehicle.status ?? "—").toLowerCase()} tone={(vehicle.status ?? "").toUpperCase() === "APPROVED" ? "ok" : "muted"} />
                  {vehicle.category ? <StatusChip label={vehicle.category.toLowerCase()} tone="muted" /> : null}
                </div>
              </div>
            ) : (
          ...
          <div className="rounded-2xl border border-token surface-1 p-4 shadow-[var(--shadow)] lg:col-span-2">
            <div className="text-sm font-semibold tracking-tight">Identity & license</div>
          ...
          {vehicle ? (
            <div className="rounded-2xl border border-token surface-1 p-4 shadow-[var(--shadow)] lg:col-span-3">
              ...
              <div className="mt-3 grid grid-cols-1 gap-2 text-xs muted md:grid-cols-3">
                ...
                <Field label="Speedometer" value={vehicle.speedometer_reading != null ? `${vehicle.speedometer_reading.toLocaleString()} km` : null} />
              </div>

              {ownerDetails ? (
                <div className="mt-4 rounded-xl border border-token bg-black/2 p-4">
                  <div className="text-xs font-semibold uppercase tracking-wide muted">Owner details</div>
                  <div className="mt-2 grid grid-cols-1 gap-1 text-xs muted md:grid-cols-3">
                    <Field label="Name" value={typeof ownerDetails["owner_full_name"] === "string" ? String(ownerDetails["owner_full_name"]) : null} />
                    <Field label="ID" value={typeof ownerDetails["owner_id_number"] === "string" ? String(ownerDetails["owner_id_number"]) : null} />
                    <Field label="Address" value={typeof ownerDetails["owner_address"] === "string" ? String(ownerDetails["owner_address"]) : null} />
                  </div>
                </div>
              ) : null}
              ...
              {companyDetails ? (
                <div className="mt-4 rounded-xl border border-token bg-black/2 p-4">
                  <div className="text-xs font-semibold uppercase tracking-wide muted">Company details</div>
                  <div className="mt-2 grid grid-cols-1 gap-1 text-xs muted md:grid-cols-3">
                    <Field label="Company" value={typeof companyDetails["company_name"] === "string" ? String(companyDetails["company_name"]) : null} />
                    <Field label="CIPC" value={typeof companyDetails["company_cipc"] === "string" ? String(companyDetails["company_cipc"]) : null} />
                    <Field label="Address" value={typeof companyDetails["company_address"] === "string" ? String(companyDetails["company_address"]) : null} />
                  </div>
                </div>
              ) : null}
            </div>
...
function Field({ label, value }: { label: string; value: string | null | undefined }) {
  return (
    <div className="rounded-xl border border-token bg-black/2 px-3 py-2">
      ...
      <div className="mt-1 truncate text-xs font-semibold text-[color:var(--foreground)]">{value?.trim() ? value : "—"}</div>
    </div>
  );
}
