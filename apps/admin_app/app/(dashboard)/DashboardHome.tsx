...
export default async function DashboardHome() {
  ...
  const [
    ...
    supabase
      .from('trips')
      .select(
        'id, status, created_at, fare_amount, payment_method, driver_id, rider_id',
      )
      .order('created_at', { ascending: false })
      .limit(6),
  ]);
  ...
  return (
    <div className='space-y-6'>
      <div className='flex flex-col gap-2 md:flex-row md:items-end md:justify-between'>
        <div>
          ...
          <p className='mt-1 text-sm muted'>
            Live operational view: active work, risk signals, and audit-ready
            actions.
          </p>
        </div>
      ...
      <div className='grid grid-cols-1 gap-3 md:grid-cols-3'>
        <KpiCard
          label='Active trips'
          value={kpiLabel(activeTrips)}
          hint='Realtime location + interventions'
          href='/trips'
        />
        <KpiCard
          label='Docs pending'
          value={kpiLabel(pendingDocs)}
          hint='Driver + vehicle verification queue'
          href='/verification'
        />
        <KpiCard
          label='Support queue'
          value='—'
          hint='Triage, escalate, close with audit'
          href='/support'
        />
      </div>
      ...
      <div className='grid grid-cols-1 gap-3 lg:grid-cols-2'>
        ...
        <Panel
          ...
          subtitle='Audited actions across the console'
          href='/audit'
        >
          <div className='divide-y divide-[color:var(--border)]'>
            {(recentAudit ?? []).map((a) => (
...
function EmptyLine() {
  return (
    <div className='py-10 text-center text-sm muted'>No activity yet.</div>
  );
}
