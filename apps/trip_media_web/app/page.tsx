  ...
  return (
    <main className='mx-auto flex min-h-svh max-w-6xl flex-col px-5 py-8 md:px-8'>
      ...
      >
        <Link
          href='/'
          className='focus-ring rounded-xl text-sm font-black uppercase tracking-[0.24em]'
        >
          Trip Media
        </Link>
        <div className='flex items-center gap-3'>
          <Link
            className='focus-ring rounded-full px-4 py-2 text-sm font-bold muted hover:text-white'
            href='/login'
          >
            Log in
          </Link>
          <Link
            className='focus-ring rounded-full bg-[var(--brand-red)] px-4 py-2 text-sm font-black text-white'
            href='/signup'
          >
            Start trial
          </Link>
      ...
      <section className='grid flex-1 items-center gap-10 py-16 md:grid-cols-[0.9fr_1.1fr]'>
        <div>
          ...
          <p className='mt-6 max-w-xl text-lg leading-8 muted'>
            Join Trip Media to prepare campaigns, manage packages, and reach
            riders through attention that happens on the move.
          </p>
          <div className='mt-8 flex flex-col gap-3 sm:flex-row '>
            <Link
              className='focus-ring inline-flex items-center justify-center gap-2 rounded-full border border-black/10 bg-gray-600 px-6 py-3 text-sm font-black text-[color:#07111f] shadow-sm hover:bg-zinc-100'
              href='/signup'
            >
              Start partner setup <ArrowRight className='size-4' aria-hidden />
            </Link>
            <Link
              className='focus-ring inline-flex items-center justify-center rounded-full border border-[var(--border)] px-6 py-3 text-sm font-black'
              href='/login'
            >
              Log in
            </Link>
